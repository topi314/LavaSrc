...
public class FloweryTTSAudioTrack extends DelegatedAudioTrack {
  ...
  public void process(LocalAudioTrackExecutor executor) throws Exception {
    try (var httpInterface = this.sourceManager.getHttpInterface()) {
      var queryParams = new URIBuilder(this.trackInfo.identifier)
        .getQueryParams();
      ...
      try (
        ...
        var streamClass = ("aac".equals(format))
          ? InputStream.class
          : SeekableInputStream.class;
        processDelegate(
          audioTrackClass
            .getConstructor(AudioTrackInfo.class, streamClass)
            .newInstance(this.trackInfo, stream),
          executor
        );
      }
    }
