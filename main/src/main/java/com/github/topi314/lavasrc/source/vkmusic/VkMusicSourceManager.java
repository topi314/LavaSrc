...
public class VkMusicSourceManager implements ExtendedAudioSourceManager, AudioLyricsManager, AudioSearchManager, BaseHttpConfigurable {
	...
	private static final String PUBLIC_API_BASE = "https://api.vk.com/method/";
	...
	private int recommendationsLoadLimit = 10;

	public VkMusicSourceManager(@NotNull String userToken) {
	...
	public AudioTrack decodeTrack(AudioTrackInfo trackInfo, DataInput input) throws IOException {
		return new VkMusicAudioTrack(trackInfo, decodeTrack(input), this);
	}
			...
			if (artistResponse != null && !artistResponse.get("response").isNull() && !artistResponse.get("response").get("items").values().isEmpty()) {
				for (var artist : artistResponse.get("response").get("items").values()) {
					...
					if (!artist.get("photo").values().isEmpty()) {
						artworkUrl = artist.get("photo").values().stream().max(Comparator.comparingLong(item -> item.get("width").asLong(0))).map(item -> item.get("url").text()).orElse(null);
					}
					artists.add(new VkMusicAudioPlaylist(artist.get("name").text() + "'s Top Tracks", Collections.emptyList(), ExtendedAudioPlaylist.Type.ARTIST, "https://vk.com/artist/" + artist.get("domain").text(), artworkUrl, artist.get("name").text(), 0));
				}
			}
		}
	...
	private List<VkMusicAudioPlaylist> parsePlaylistSearch(List<JsonBrowser> playlistItems, ExtendedAudioPlaylist.Type type) {
		return playlistItems.stream().map(playlistItem -> new VkMusicAudioPlaylist(playlistItem.get("title").text(), Collections.emptyList(), type, "https://vk.com/music/" + (type == ExtendedAudioPlaylist.Type.PLAYLIST ? "playlist" : "album") + "/" + playlistItem.get("owner_id").text() + "_" + playlistItem.get("id").text() + (playlistItem.get("access_key").isNull() ? "" : "_" + playlistItem.get("access_key").text()), this.parsePlaylistThumbnail(playlistItem), this.parseAlbumAuthor(playlistItem), playlistItem.get("count").as(Integer.class))).collect(Collectors.toList());
	}
	...
	private BasicAudioLyrics parseTimestampsLyrics(@NotNull List<JsonBrowser> lines) {
		...
		var text = lines.stream().map(line -> {
			strokes.add(new BasicAudioLyrics.BasicLine(Duration.ofMillis(line.get("begin").as(Long.class)), Duration.ofMillis(line.get("end").as(Long.class) - line.get("begin").as(Long.class)), line.get("line").text()));
	...
	private BasicAudioLyrics parseTextLyrics(@NotNull List<JsonBrowser> lines) {
		var strokes = new ArrayList<AudioLyrics.Line>();
		var text = lines.stream().filter(line -> !line.text().isEmpty()).map(line -> {
	...
	private AudioItem getPlaylist(String owner_id, String playlist_id, String accessKey) throws IOException {
		...
		String title = null;
		var playlistJson = this.getJson("audio.getPlaylistById", query + "&playlist_id=" + playlist_id).get("response");
		...
		return new VkMusicAudioPlaylist(title, tracks, type ? ExtendedAudioPlaylist.Type.ALBUM : ExtendedAudioPlaylist.Type.PLAYLIST, "https://vk.com/music/" + (type ? "album" : "playlist") + "/" + owner_id + "_" + playlist_id + (playlistJson.get("access_key").isNull() ? "" : "_" + playlistJson.get("access_key").text()), coverUri, this.parseAlbumAuthor(playlistJson), tracks.size());
	}

	private String parseAlbumAuthor(JsonBrowser json) {
		if (!json.get("main_artists").values().isEmpty()) {
			return json.get("main_artists").values().stream().map(node -> node.get("name").text()).collect(Collectors.joining(", "));
		}
		return null;
	}
	...
	private AudioItem getArtist(String id) throws IOException {
		...
		var artistJson = this.getJson("audio.getArtistById", "&artist_id=" + id).get("response");
	...
	private AudioTrack parseTrack(JsonBrowser json) {
		try {
			...
			if (!json.get("main_artists").isNull() && !json.get("main_artists").values().isEmpty() && !json.get("main_artists").values().get(0).get("domain").isNull()) {
			...
			return new VkMusicAudioTrack(new AudioTrackInfo(json.get("title").text(), json.get("artist").text(), json.get("duration").as(Long.class) * 1000, audioId, false, "https://vk.com/audio" + audioId, coverUri, null), new ExtendedAudioTrackInfo(albumTitle, albumUrl, artistUrl, null, false), this);
		} catch (Exception e) {
			return null;
		}
	}
