...
public class StringCompareMirroringAudioTrackResolver implements MirroringAudioTrackResolver {
	...
	public StringCompareMirroringAudioTrackResolver(String[] sources, String[] providers, Float titleThreshold, Float authorThreshold, Float totalMatchThreshold, boolean skipSoundCloudGo, Float levelOnePenalty, Float levelTwoPenalty, Float levelThreePenalty) {
	...
	public int calculateMatchScore(String targetTitle, String targetAuthor, String candidateTitle, String candidateAuthor, long targetDuration, long candidateDuration) {
		...
		log.info("Track Scored: {} : {} with score : {} :: Title Score : {} :: Title Match Ratio : {} :: Author Score : {} :: Duration Score : {} :: Penalty : {}", candidateTitle, candidateAuthor, totalScore, titleScore, titleMatchRatio, authorScore, durationScore, penalty);
	...
	public AudioItem applyStringComparison(AudioItem item, MirroringAudioTrack track, String provider) {
		...
		if (this.skipSoundCloudGo) {
			tracks = tracks.stream().filter(t -> !isSoundCloudGoTrack(t)).collect(Collectors.toList());
		...
		if (sourcePrefix.length > 0) {
			for (var source : sources) {
				if (sourcePrefix[0].equals(source)) {
					...
					List<AudioTrack> matchedTracks = scoredTracks.stream().map(Pair::getFirst).collect(Collectors.toList());
	...
	private List<Pair<AudioTrack, Integer>> scoreTracks(List<AudioTrack> tracks, AudioTrackInfo audioTrackInfo) {
		...
		for (AudioTrack track : tracks) {
			int score = this.calculateMatchScore(audioTrackInfo.title, audioTrackInfo.author, track.getInfo().title, track.getInfo().author, audioTrackInfo.length, track.getInfo().length);
	...
	private int getMaxScore(List<Pair<AudioTrack, Integer>> scoredTracks) {
		return scoredTracks.stream().map(Pair::getSecond).max(Integer::compareTo).orElse(0);
	}
