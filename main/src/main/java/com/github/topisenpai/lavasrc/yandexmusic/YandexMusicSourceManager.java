...
public class YandexMusicSourceManager implements AudioSourceManager, HttpConfigurable {
    ...
    public static final String SEARCH_PREFIX = "yasearch:";
    ...
    public YandexMusicSourceManager(String token, AudioPlayerManager audioPlayerManager) {
        if (token == null || token.isEmpty()) {
            throw new IllegalArgumentException("Yandex Music token must be set");
        }
        this.token = token;
    ...
    public AudioItem loadItem(AudioPlayerManager manager, AudioReference reference) {
        try {
            ...
            switch (matcher.group("type1")) {
                ...
                case "users":
                    var user = matcher.group("identifier");
                    var playlistId = matcher.group("identifier2");
                    return this.getPlaylist(user, playlistId);
                case "artist":
    ...
    private AudioItem getSearch(String query) throws IOException {
        ...
        if (json == null || json.get("result").get("tracks") == null)
    ...
    public JsonBrowser getJson(String uri) throws IOException {
        ...
        request.setHeader("Authorization", "OAuth " + this.token);
    ...
    public void encodeTrack(AudioTrack track, DataOutput output) throws IOException {
        ...
        DataFormatTools.writeNullableText(output, yandexMusicAudioTrack.getSongId());
