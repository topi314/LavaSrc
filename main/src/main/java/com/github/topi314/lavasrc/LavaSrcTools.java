...
public class LavaSrcTools {
  ...
  @Nullable
  public static JsonBrowser fetchResponseAsJson(
    ...
    try (CloseableHttpResponse response = httpInterface.execute(request)) {
      ...
      if (statusCode == HttpStatus.SC_NOT_FOUND) {
        var data = IOUtils.toString(
          response.getEntity().getContent(),
          StandardCharsets.UTF_8
        );
        log.error(
          "Server responded with not found to '{}': {}",
          request.getURI(),
          data
        );
        return null;
        ...
        log.error(
          "Server responded with an error to '{}': {}",
          request.getURI(),
          data
        );
    ...
    public static Object fetchResponseAsJson(CloseableHttpResponse response) {
        return null;
    }
}
