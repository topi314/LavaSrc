...
public class AppleMusicSourceManager
  ...
  public static final Set<AudioSearchResult.Type> TOP_RESULT_SEARCH_TYPES = Set.of(
    AudioSearchResult.Type.TRACK,
    AudioSearchResult.Type.ALBUM,
    AudioSearchResult.Type.PLAYLIST,
    AudioSearchResult.Type.ARTIST
  ...
  @Override
  public @Nullable AudioSearchResult loadSearch(
  ...
  public void parseTokenData() throws IOException {
    ...
    var json = JsonBrowser.parse(
      new String(Base64.getDecoder().decode(this.token.split("\\.")[1]))
    );
  ...
  public AudioSearchResult getSearchSuggestions(
    String query,
    ...
    var urlBuilder = new URIBuilder(
      API_BASE + "catalog/" + countryCode + "/search/suggestions"
    );
          ...
          case "albums":
            {
              ...
              var album = new AppleMusicAudioPlaylist(
                name,
          ...
          case "artists":
            {
              var name = attributes.get("name").text();
              var artworkUrl = parseArtworkUrl(attributes.get("artwork"));
              var artist = new AppleMusicAudioPlaylist(
                name + "'s Top Tracks",
                Collections.emptyList(),
          ...
          case "playlists":
            {
              ...
              var author = attributes
                ...
                .get("curatorName")
                .text();
              var playlist = new AppleMusicAudioPlaylist(
                name,
                Collections.emptyList(),
          ...
          case "songs":
            {
              ...
              var track = new AppleMusicAudioTrack(
                info,
                albumName,
                albumUrl,
                artistUrl,
  ...
  public Map<String, String> getArtistCover(List<String> ids)
    throws IOException {
    if (ids.isEmpty()) {
      return Map.of();
    }
  ...
  public AudioItem getSearch(String query, boolean preview) throws IOException {
    ...
    if (
      json == null ||
      json.get("results").get("songs").get("data").values().isEmpty()
    ) {
      return AudioReference.NO_TRACK;
    }
    return new BasicAudioPlaylist(
      "Apple Music Search: " + query,
      this.parseTracks(json.get("results").get("songs"), preview),
      null,
      true
    );
  }

  public AudioItem getAlbum(String id, String countryCode, boolean preview)
    throws IOException {
    var json =
      this.getJson(
          API_BASE +
    ...
    var artworkUrl =
      this.parseArtworkUrl(
          json.get("data").index(0).get("attributes").get("artwork")
        );
    var author = json
      .get("data")
      .index(0)
    ...
    return new AppleMusicAudioPlaylist(
      json.get("data").index(0).get("attributes").get("name").text(),
      tracks,
      ExtendedAudioPlaylist.Type.ALBUM,
      json.get("data").index(0).get("attributes").get("url").text(),
      artworkUrl,
      author,
      (int) json
        .get("data")
  ...
  public AudioItem getPlaylist(String id, String countryCode, boolean preview)
    throws IOException {
    var json =
      this.getJson(API_BASE + "catalog/" + countryCode + "/playlists/" + id);
    if (json == null) {
      return AudioReference.NO_TRACK;
    }
  ...
  public AudioItem getArtist(String id, String countryCode, boolean preview)
    throws IOException {
    var json =
      this.getJson(
          API_BASE +
    ...
    var jsonArtist =
      this.getJson(API_BASE + "catalog/" + countryCode + "/artists/" + id);

    var author = jsonArtist
      .get("data")
      .index(0)
      .get("attributes")
      .get("name")
    ...
    var artworkUrl =
      this.parseArtworkUrl(
          jsonArtist.get("data").index(0).get("attributes").get("artwork")
        );
    var artistArtwork = new HashMap<String, String>();
    if (artworkUrl != null) {
      artistArtwork.put(
        jsonArtist.get("data").index(0).get("id").text(),
        artworkUrl
      );
    }
  ...
  public AudioItem getSong(String id, String countryCode, String route, boolean preview) throws IOException { //ghetto ahhh code should prob do its method for the GetVideo
    ...
    if (artistId != null) {
      artistArtwork =
        getArtistCover(List.of(artistId)).values().iterator().next();
    }
    return parseTrack(json.get("data").index(0), preview, artistArtwork);
  }

  private List<AudioTrack> parseTracks(
    JsonBrowser json,
    boolean preview,
    Map<String, String> artistArtwork
  ) {
    var tracks = new ArrayList<AudioTrack>();
  ...
  private List<AudioTrack> parseTrackList(JsonBrowser json, boolean preview)
    throws IOException {
  ...
  private AudioTrack parseTrack(
    ...
    if (
      ...
      (
        artistUrl.isEmpty() ||
        artistUrl.startsWith(
          "https://music.apple.com/WebObjects/MZStore.woa/wa/viewCollaboration"
        )
      )
    ) {
      artistUrl = null;
  ...
  private String parseArtworkUrl(JsonBrowser json) {
    ...
    return text
      .replace("{w}", json.get("width").text())
      .replace("{h}", json.get("height").text());
  }
